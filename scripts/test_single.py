...
############################################################
save_dir = os.path.join(params.exp_dir, f'test_single_{args.sample_std}_trail_{args.trail}')
...
def batch_psnr(sam, gt):
    ...
    H, W, C = sam.shape[1:]
    err = np.sum((sam - gt)**2, axis=(1, 2, 3))
    err /= (H * W * C)
...
for i in range(num_batches):
    ll, sam, gt, mask = sess.run(
        [model.test_ll, model.test_sam_mean, testset.x, testset.m])
    log_likel.append(ll)
...
f_beta = prd.prd_to_max_f_beta_pair(prd_data[0], prd_data[1], beta=8)
