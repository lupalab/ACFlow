...
def embed_images_in_inception(imgs, inception_path, layer_name, batch_size=100):
    ...
    with tf.Session() as sess:
        while i < len(imgs):
            input_nda = imgs[i:i + batch_size]
            if input_nda.shape[-1] == 1:
                input_nda = np.concatenate([input_nda] * 3, axis=-1)
            embeddings.append(sess.run(
                feature_tensor, feed_dict={input_tensor: input_nda}))
            i += batch_size
